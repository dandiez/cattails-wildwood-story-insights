...
from cws_insights.merged_data.items import ItemPlus, AllItemPlus, get_merged_item_data, UidStem
...
def _npc_nice_names(npc_stem_ids: list[UidStem], all_data: AllResourceData):
    ...
    return [all_data.npcs_lang_english_lang[npcstem].lang_npc_name for npcstem in npc_stem_ids]
...
def yield_lines_npc_block(i: ItemPlus, all_data: AllResourceData):
    ...
    if any(asdict(from_npcs).values()):
        ...
        if from_npcs.npc_loves:
            yield f"NPCs that love {i.item_lang.lang_item_name}: " + ", ".join(_npc_nice_names(from_npcs.npc_loves, all_data))
        if from_npcs.npc_likes:
            yield f"NPCs that like {i.item_lang.lang_item_name}: " + ", ".join(_npc_nice_names(from_npcs.npc_likes, all_data))
        if from_npcs.npc_dislikes:
            yield f"NPCs that dislike {i.item_lang.lang_item_name}: " + ", ".join(_npc_nice_names(from_npcs.npc_dislikes, all_data))
        if from_npcs.npc_hates:
            yield f"NPCs that hate {i.item_lang.lang_item_name}: " + ", ".join(_npc_nice_names(from_npcs.npc_hates, all_data))
...
def write_item_merged_data(item_merged: AllItemPlus, wiki_contents_dir: str, all_resource_data: AllResourceData):
...
def main(gameresources_dir, wiki_contents_dir):
    ...
    write_item_merged_data(item_merged, os.path.join(wiki_contents_dir, "items"), all_resource_data)
