...
class IndexModule:
    ...
    def write_index(self, schemas_dir: str):
        module_as_str = self.get_index_module_code()
        with open(os.path.join(schemas_dir, "_index.py"), "w") as f:
            f.write(module_as_str)

    def get_index_module_code(self) -> str:
        self._sort_mapping()
...
def write_schema_as_dataclass(
    resources_rel_path: ResourcesRelPath,
    collection: list[ResourceFile],
    output_dir: str,
) -> ModuleClassNames:
    dataclass_code = get_python_module_with_dataclass_as_str(
        collection, resources_rel_path
    )
    module_name = slug_it(resources_rel_path.lower())
